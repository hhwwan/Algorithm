...
2021년 1월 5일에 수집된 개인정보가 A약관으로 수집되었다면 해당 개인정보는2022년 1월 4일까지 보관 가능하며 2022년 1월 5일부터 파기해야 할 개인정보입니다.
...
def solution(today, terms, privacies):
    ...
    today_days = days_calculate(today)
    
    for i in terms:
        types, duration = i.split()
        term_dict[types] = int(duration) * 28
    
    for j in privacies:
        date, p_types = j.split()
        final_days = days_calculate(date) + term_dict[p_types] - 1
        if final_days < today_days:
            answer.append(cnt)
        cnt += 1
    return answer