...
def solution(files):
    ...
    for name in files:
        match = re.match(r'([^\d]*)(\d+)(.*)', name) # 숫자 나오기전, 숫자, 그 뒤
        if match:
            head.append(match.group(1))
            number.append(match.group(2))
            tail.append(match.group(3))
    
    for i in range(len(head) - 1):
        for j in range(i + 1, len(head)):
            # head 문자열이 앞이면 그대로 유지
            if head[i].lower() < head[j].lower():
                continue
            # head 문자열이 동일하다면 number 비교
            elif head[i].lower() == head[j].lower():
                if int(number[i]) <= int(number[j]):
                    continue
                # number 숫자가 뒤라면 모두 변경
                else:
    ...
    for i in range(len(head)):
        answer.append(head[i] + number[i] + tail[i])
    return answer
...
def solution(files):
    ...
    for name in files:
        ...
        if match:
            ...
            tail = match.group(3)
            answer.append((head, number, tail, name))
