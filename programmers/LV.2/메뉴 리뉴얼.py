...
def solution(orders, course):
    ...
    for num in course:
        comb = [] # 조합을 담을 리스트
        for food in orders:
            sort_food = sorted(food) # 정렬 리스트 생성
            comb.extend(combinations(sort_food, num))
        
        count = Counter(comb)
    
        if count:
            max_cnt = max(count.values()) # 최댓값
            if max_cnt >= 2:
                for comb, cnt in count.items():
                    print(comb, cnt)
                    if cnt == max_cnt: # 2개가 넘는 것들 중 최댓값
