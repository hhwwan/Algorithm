...
제외하도록 이벤트 당첨자 담당자인 "프로도" 에게 전달하려고 합니다. 
...
def solution(user_id, banned_id):
    answer = set() # 재제 아이디 조합 저장
    
    # 아이디 매칭 되는지 확인
    def match(user, ban):
        if len(user) != len(ban):
            return False
        for u_i, b_i in zip(user, ban):
            if b_i != '*' and u_i != b_i:
                return False
        return True
    ...
    def dfs(idx, pick):
        if idx == len(banned_id):
            answer.add(frozenset(pick))
            return
        
        for uid in user_id:
            if uid in pick: # 이미 비교 한거면 continue
                continue
            if match(uid, banned_id[idx]):
                dfs(idx+1, pick + [uid])
